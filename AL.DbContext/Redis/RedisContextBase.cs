...
namespace AL.DbContext.Redis
{
    /// <summary>
    /// Redis缓存数据库处理
    /// </summary>
    public class RedisContextBase : IDisposable
    {
        /// <summary>
        /// 数据库编号
        /// </summary>
        ...
        public RedisContextBase(string connStringName) : this(connStringName, 0) { }
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="connStringName">配置文件连接字符串名称</param>
        /// <param name="dbNum">数据库编码</param>
        public RedisContextBase(string connStringName, int dbNum)
        {
            if (string.IsNullOrWhiteSpace(connStringName))
                throw new Exception("请设置配置文件连接字符串名称");
            DbNum = dbNum;
            _conn = RedisConnection.GetConnectionMultiplexer(connStringName);
        }
